 ...
 * 同一台主机上的两项服务若使用环回地址而非分配的主机地址，就可以绕开TCP/IP协议栈的下层,立即转发到本机IP层进行处理
 */

- (void)viewDidLoad {
    [super viewDidLoad];
    //iOS端调试模式  YES: 模拟器调试(Mac/iOS Simulator在同一主机), NO: 真机调试(Mac/iPhone不是同一主机)
    BOOL _isSimulator = YES;
    if (_isSimulator) {
        _loc_ipAdr = @"127.0.0.1";//本机地址 是 Mac环回地址
        _loc_port = @"10000";
        
        _des_ipAdress = [self getIPAddress];//目标IP地址
        _des_port = @"10001";
    } else {
        _loc_ipAdr = [self getIPAddress];//本机地址 是 IP地址
        _loc_port = @"10000";
        
        _des_ipAdress = @"10.210.24.40";//目标IP地址
        _des_port = @"10001";
    }
