...
- (void)viewDidLoad {
    [super viewDidLoad];
    //iOS端调试模式  YES: 模拟器调试, NO: 真机调试
    BOOL _isSimulator = YES;
    if (_isSimulator) {
        _loc_ipAdr = [self getIPAddress];//本机IP地址
        _loc_port = @"10001";
        
        _des_ipAdress = @"127.0.0.1";//目标地址 是 Mac环回地址
        _des_port = @"10000";
    } else {
        _loc_ipAdr = [self getIPAddress];//本机IP地址
        _loc_port = @"10001";
        
        _des_ipAdress = @"10.210.25.111";//目标IP地址
        _des_port = @"10000";
    }
...
//    bzero(&loc_addr, sizeof(loc_addr));
    ...
    //绑定
    int udpCode = bind(_udp_clientSockfd, (const struct sockaddr *)&loc_addr,sizeof(loc_addr) );
